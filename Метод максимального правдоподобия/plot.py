...
SHIFT = 5.0  # именно то самое "+5" как у тебя в C++

# Тики как на скриншотах
...
def y_from_p_weibull(p: float) -> float:
    # ровно как в твоём C++: 5 + ln ln(1/(1-p))
...
def plot_into_axis(ax, xout_path: str) -> None:
    ...
    if is_weibull:
        # Слева: Zp+5 (по смыслу у тебя это Weibull-paper + 5, но подпись как на скрине)
        ax.set_ylabel("Zp+5")

        # Если похоже на скрин — шаги
        ...
        # Normal: слева сразу P% (как на первом скрине)
        ax.set_ylabel("P%")
